...
Live at: `https://truehour.tabajahstack.com`
...
```
website/
├── astro.config.mjs          # Astro config — site URL, output dir, integrations
...
└── src/
    ├── consts.ts              # APP_NAME, DOWNLOAD_URL, dynamic version fetching from GitHub API
    ├── env.d.ts               # Type declarations for GSAP and Three.js
    ├── assets/
    │   └── dashboard-screenshot.png   # App UI preview image
    ├── components/
    │   ├── Navbar.astro       # Fixed nav — theme toggle, mobile menu, anchor navigation
    │   ├── DownloadButton.astro # Shared primary download CTA component
    ...
    │   └── navbar.ts          # Navbar behavior (scroll state, mobile menu, theme toggle)
    └── styles/
...
- **Site**: `https://truehour.tabajahstack.com`
...
- No direct download button; Download anchor link scrolls users to the dedicated download section
- Responsive mobile hamburger menu with backdrop blur and max-height animation
- External `src/scripts/navbar.ts` module handles scroll detection, theme persistence (`localStorage`), and mobile menu toggling
...
- Dynamic version number (fetched once at page level and passed as props)
...
`src/consts.ts` exports both `getAppVersion()` and `DOWNLOAD_URL`.

`getAppVersion()` fetches the latest release tag from the GitHub API at build time:
...
`src/pages/index.astro` fetches the version once and passes it to Base, Hero, and Download.

### CTA Placement Strategy

- Primary download buttons are intentionally limited to two placements: Hero and Download sections
- Navbar should remain navigation-only and point to `#download` instead of repeating download CTAs
- Download URL must be sourced from `DOWNLOAD_URL` in `src/consts.ts` to avoid duplication
- Shared button styling and markup should go through `src/components/DownloadButton.astro`
